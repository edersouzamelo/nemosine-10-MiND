...
# FastAPI app
# -----------------------------
app = FastAPI(title="MiND — Minimal Deterministic Middleware", version=__version__)
...
REGISTRY = JsonlRegistry(
    path=os.path.join(os.path.dirname(__file__), "runtime", "cycles.jsonl")
)
MOTOR = OpenAIMotor(
...
@app.get("/ame/config")
def get_config():
    # Configuração é EXTERNA ao motor e inspecionável
