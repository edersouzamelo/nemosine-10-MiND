...
The proposed software is not an autonomous agent framework. It does not perform goal formulation, multi-step planning, self-directed action, or tool invocation loops. Instead, it operates as a deterministic middleware layer that mediates each interaction with a language model in an explicit, reproducible, and externally controlled manner.
